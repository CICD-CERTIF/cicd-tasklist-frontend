pipeline {
    ...
    environment {
        // Tes variables d'environnement personnalisées
        DOCKER_USERNAME       = 'ndongmo'
        DOCKER_CREDENTIALS_ID = 'wilfrid-dockerhub-password'
        SONAR_HOST_URL        = 'https://sonarqube.cicd.kits.ext.educentre.fr'
        SONAR_PROJECT_KEY     = 'wilfrid-cicd-tasklist-frontend'
    }
        ...
        stage('SonarQube analysis') {
            steps {
                withSonarQubeEnv('sonar-wilfrid') {
                    withCredentials([
                        string(credentialsId: 'wilfrid-sonar-token-frontend', variable: 'SONAR_TOKEN')
                    ]) {
                        sh '''
